...
# There are 3 major concern with redux that redux-toolkit solve
 ...
 2) i have to add lot of packages to get Redux to do anything usefull
 3) Redux require to much boilaplate code
 4) Redux is So complicate , it require lot of learning initial curve

 2) Explain Dispatcher

- Dispatching an action is the process of sending that action to the Redux store. It's done using the dispatch method provided by the store is called Dispatcher

- Basically when you clicked a button , behind the seen it dispatches an action to perform the certain function this function modified the perticular slice of the store

3) Explain Reducer 
 
- In Redux Toolkit, a reducer is a function that specifies how the application's state changes in response to dispatched actions. A reducer takes the current state and an action as arguments, and it returns a new state. Reducers are responsible for updating specific parts of the state based on the action type and payload.

- Redux Toolkit provides a utility function called createSlice that simplifies the process of creating reducers, actions, and initial state for a specific "slice" of the Redux state. A "slice" represents a portion of the overall state that has its own actions and reducer logic.

4) Explain Slice 

- In Redux Toolkit, a "slice" refers to a specific part or section of the Redux state that has its own reducer logic, initial state, and set of actions. Slices are created using the createSlice function provided by Redux Toolkit. A slice encapsulates related pieces of the Redux store, making it easier to manage and organize the state and actions in a modular way.

5) Explain CreateSlice and the configration it takes ?

# Reducer Logic:
The primary purpose of a slice is to define how the state should change in response to dispatched actions. This is done through reducer functions. Reducers take the current state and an action, and they return a new state based on the action type and payload.

# Initial State:
Each slice has its own initial state, representing the default values for the properties managed by that slice.

# Actions:
Actions are a set of functions that generate and describe the actions that can be dispatched to the Redux store. These actions are often automatically generated by createSlice.

# Action Types:
createSlice automatically generates action types for each action, making it easier to reference them in other parts of the application.

# Action Creators:
Action creators are functions generated by createSlice that simplify the process of creating actions with the correct structure.
